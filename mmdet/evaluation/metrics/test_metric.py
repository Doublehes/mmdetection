...
class TestMetric(BaseMetric):
    ...
    default_prefix: Optional[str] = 'test'
    ...
    def compute_metrics(self, results: list) -> dict:
        ...
        if self.metric == 'mAP':
            ...
            area_ranges = [[0 ** 2, 30 ** 2], [30 ** 2, 60 ** 2], [60 ** 2, 1e5 ** 2]]
            ...
            for i, (gt, pred) in enumerate(zip(gts, preds)):
                for area_range, area_label in zip(area_ranges, area_labels):
                    gt_ = copy.deepcopy(gt)
                    gt_area = (gt_['bboxes'][:, 2] - gt_['bboxes'][:, 0] + 1) * (gt_['bboxes'][:, 3] - gt_['bboxes'][:, 1] + 1)
                    gt_mask = (gt_area > area_range[0]) & (gt_area <= area_range[1])
                    gt_['bboxes'] = gt_['bboxes'][gt_mask]
                    gt_['labels'] = gt_['labels'][gt_mask]
                    objects_dict[area_label]['gts'].append(gt_)

                    pred_ = copy.deepcopy(pred)
                    pred_selected = []
                    for pred_objs in pred_:  # pred_objs: [x1, y1, x2, y2, score]
                        pred_objs_mask = (pred_objs[:, 2] - pred_objs[:, 0] + 1) * (pred_objs[:, 3] - pred_objs[:, 1] + 1)
                        pred_objs_mask = (pred_objs_mask > area_range[0]) & (pred_objs_mask <= area_range[1])
                        pred_objs = pred_objs[pred_objs_mask]
                        pred_selected.append(pred_objs)
                    objects_dict[area_label]['preds'].append(pred_selected)
            iou_thr = 0.5
            for area_label in area_labels:
                logger.info(f'\n{"-" * 15}iou_thr: {iou_thr}, area: {area_label}{"-" * 15} ')
                gts_ = objects_dict[area_label]['gts']
                preds_ = objects_dict[area_label]['preds']
                mean_ap, _ = eval_map(
                    preds_,
                    gts_,
                    scale_ranges=self.scale_ranges,
                    ...
                    eval_mode=self.eval_mode,
                    use_legacy_coordinate=True)
                eval_results[f'AP{int(iou_thr * 100):02d}_{area_label}'] = round(mean_ap, 3)
